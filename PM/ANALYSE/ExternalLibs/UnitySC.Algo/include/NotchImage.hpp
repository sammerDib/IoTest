...
public:
  ...
  using ImageCenter = NamedType<cv::Point2d, ImageCenterTag>;
   ...
   */
  Algorithms::Status GetRotation(double &angle, cv::Point2d waferCenter, std::string const &reportPath);

private:
  explicit NotchImage::NotchImage(std::string const &imageFileName, PixelSize const &pixelSize, ImageCenter const &centroid)
      : EdgeImage(imageFileName, pixelSize, centroid) {}

  explicit NotchImage::NotchImage(std::string const &imageName, cv::Mat image, PixelSize const &pixelSize, ImageCentroid const &centroid)
