...
cv::Mat PolarImageCropper::Crop(cv::Mat &image, cv::Rect *boundingRect) {
  ...
  if (allBlackImage) {
    ...
    double minimalSumForConsideredGoodLine = image.cols * 0.96;
    ...
    double minimalSumForConsideredGoodColumn = image.rows * 0.96;
    ...
    if (nullptr != boundingRect) {
      ...
      Logger::Warning("[PolarImageCropper::Crop] Please pass 'boundingRect' parameter to Crop()");
