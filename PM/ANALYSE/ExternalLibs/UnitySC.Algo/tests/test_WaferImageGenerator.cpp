...
namespace {
  ...
  WaferImageGenerator::Parameters GetParametersForTestTiles() {
    ...
    params.imagesToTake.emplace_back("upleft", WaferImageGenerator::ImageType::EDGE, cv::Point2i(-256 * params.pixelSize.x, 256 * params.pixelSize.y));
    params.imagesToTake.emplace_back("upright", WaferImageGenerator::ImageType::EDGE, cv::Point2i(256 * params.pixelSize.x, 256 * params.pixelSize.y));
    params.imagesToTake.emplace_back("bottomright", WaferImageGenerator::ImageType::EDGE, cv::Point2i(256 * params.pixelSize.x, -256 * params.pixelSize.y));
    params.imagesToTake.emplace_back("bottomleft", WaferImageGenerator::ImageType::EDGE, cv::Point2i(-256 * params.pixelSize.x, -256 * params.pixelSize.y));
...
TEST(WaferImageGenerator, Expect_pixelSize_to_be_considered) {
  ...
  EXPECT_TRUE(result.pixelSize == pixelSize) << "Result pixelSize must be the one provided in Generate() call";
}

TEST(WaferImageGenerator, Expect_radius_to_be_considered) {

  WaferImageGenerator::Parameters params = GetParametersFor300mm2x();
  const int radiusFor300mm = 150000;
  WaferImageGenerator generator;
  WaferImageGenerator::Result result = generator.Generate(params);
  EXPECT_TRUE(result.radius == radiusFor300mm) << "Result wafer radius must be the one provided in Generate() call";
}

TEST(WaferImageGenerator, Expect_notch_rotation_to_be_right) {
  ...
  for (auto rotationAngle : angles) {
    ...
    EXPECT_LT(notch, 127) << "Failed for " + std::to_string(rotationAngle) + " at (" + std::to_string(expectedNotchCartesianPoint.x) + "," + std::to_string(expectedNotchCartesianPoint.y) + ")";
  }
}

TEST(WaferImageGenerator, Expect_pixel_shift_to_be_considered) {
  ...
  EXPECT_TRUE(result.pixelShift == params.waferShift) << "Result wafer shift must be the one provided in Generate() call";
}

TEST(WaferImageGenerator, Expect_test_tiles) {
  ...
  EXPECT_TRUE(result.takenImages.size() == 4) << "We should have 4 quadrant images";
  ...
  EXPECT_EQ(differentPixeCount, 0) << "There must be no differences between generated and expected images";
}
