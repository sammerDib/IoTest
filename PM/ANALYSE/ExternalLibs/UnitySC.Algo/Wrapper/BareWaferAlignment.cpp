...
namespace AlgosLibrary {
  ...
  BareWaferAlignmentResult ^ BareWaferAlignment::PerformBareWaferAlignment(List<BareWaferAlignmentImageData^> ^ images, WaferInfos^ waferData, String ^ reportPath) {
    ...
    if (edgeImages->Count < BareWaferAlignment::MINIMAL_IMAGES_FOR_BWA) {
      ...
      if (WaferType::NOTCH == waferData->Type) {
        if (notchImages->Count == 1) {
          ...
          std::string cppStringReportPath(StringToCharArray(reportPath));
          Algorithms::Status status = wafer.GetGeometricalParameters(&geoMeasure, cppStringReportPath);
          result->Status->Confidence = status.confidence;

          if (Algorithms::StatusCode::OK == status.code) {
            double angle;
            status = wafer.GetMisalignmentAngle(angle, geoMeasure.centerShift, cppStringReportPath);
  ...
  ContourExtractionResult ^ BareWaferAlignment::PerformEdgeImageContourExtraction(BareWaferAlignmentImageData^ imageData, String ^ reportPath) {
    ...
    if (imageData->ExpectedShape == WaferEdgeShape::NOTCH) {
      ...
      for (auto const &point : edgeImage->GetContourPoints(_reportPath)) {
