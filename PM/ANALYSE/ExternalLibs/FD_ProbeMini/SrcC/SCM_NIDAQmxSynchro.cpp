...
int SPG_CONV SCM_NIDAQmxSynchronize(int MasterHandle, int SlaveHandle)
{
	...
	uInt32 synchType=0; //6133 = Serie S

	TaskHandle& masterTaskHandle=*(TaskHandle*)&MasterHandle;
	TaskHandle& slaveTaskHandle=*(TaskHandle*)&SlaveHandle;
...
static int32 GetTerminalNameWithDevPrefix(TaskHandle taskHandle, const char terminalName[], char triggerName[])
{
	...
	//cette fonction est utilisée par les exemples mais ni déclarée ni documentée dans le SDK //http://forums.ni.com/t5/Multifunction-DAQ/DAQmxGetNthTaskDevice-is-UNDOCUMENTED-but-used-in-an-example/m-p/504198 //http://pyrwi.googlecode.com/svn-history/r3/trunk/nidaqmx/nidaqmx.i
	//rajouté manuellement dans SPG_NIDAQmxConfig
	...
	while( i<=numDevices ) {
		...
		if( productCategory!=DAQmx_Val_CSeriesModule && productCategory!=DAQmx_Val_SCXIModule ) { //http://forums.ni.com/t5/Multifunction-DAQ/check-GetTerminalNameWithDevPrefix/td-p/479083/page/2
			*triggerName++ = '/';
...
//Exemple du SDK NIDAQmx C:\Users\Public\Documents\National Instruments\NI-DAQ\Examples\DAQmx ANSI C\Synchronization\Multi-Device
...
int32 CVICALLBACK EveryNCallback(TaskHandle taskHandle, int32 everyNsamplesEventType, uInt32 nSamples, void *callbackData)
{
	...
	printf("\t%d\t%d\t\t%d\t%d\r",masterRead,slaveRead,masterTotal,slaveTotal);
	fflush(stdout);
