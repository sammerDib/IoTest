...
#define sci_UID sci_UID_EmuleLISEED
#define sci_NAME sci_NAME_EmuleLISEED
...
static SCX_CONNEXION* SPG_CONV scxELISEEDOpen(SCI_CONNEXIONINTERFACE* CI, SCX_ADDRESS* Address, int scxOpenFlag)
{
	sciCHECK(CI,"scxOpen");
	...
	S_InitTimer(State.T,"scxELISEEDOpen");
...
static int SPG_CONV scxELISEEDClose(SCX_CONNEXION* C)
{
	scxCHECK(C, "scxClose");
	SCX_STATE& State=*C->State;
	C->Etat=scxINVALID;
...
static int SPG_CONV scxELISEEDWrite(void* Data, int DataLen, SCX_CONNEXION* C)
{
	scxCHECK(C, "scxWrite");
	SCX_STATE& State=*C->State;
...
static int SPG_FASTCONV scxLISEEDGenerateSampleX(SCX_STATE& State, SIGNAL_POSITION& SPos)
{
...
static int SPG_FASTCONV scxLISEEDGenerateSampleI(SCX_STATE& State, SIGNAL_POSITION& SPos)
{
	double Rampe=2*SPos.NormalizedPos;
	if(Rampe>1) Rampe=2-Rampe;
...
static int SPG_CONV scxELISEEDRead(void* Data, int& DataLen, SCX_CONNEXION* C)
{
	scxCHECK(C, "scxRead");
	SCX_STATE& State=*C->State;
	...
	CHECK(SamplesToWrite>State.Address.BufferSamples,
				"scxELISEEDRead: Emulating internal overwrite",
					S_GetTimerRunningCount(State.T,State.TRead);
					SPOS_Increment(State.Sampled,State.Address.FrequencyHz,State.TRead);
					State.Written=State.Sampled;
			return DataLen=0);
	
	DbgCHECK(DataLen&3,"scxELISEEDRead: Incorrect buffer size");
	SamplesToWrite=V_Min(SamplesToWrite,DataLen/4);//4 = X:2 I:2
	...
	return DataLen=SamplesToWrite*4;
}

// ###################################################

static void SPG_CONV scxELISEEDCfgAddress(SPG_CONFIGFILE& CFG, SCX_ADDRESS* Address, int Flag)
{
...
static int SPG_CONV scxELISEEDSetParameter(char* ParamName, void* Value, SCX_CONNEXION* C)
{
...
static int SPG_CONV scxELISEEDGetParameter(char* ParamName, void* Value, SCX_CONNEXION* C)
{
...
static int SPG_CONV sciELISEEDDestroyConnexionInterface(SCI_CONNEXIONINTERFACE* CI)
{
...
SCI_CONNEXIONINTERFACE* SPG_CONV sciELISEEDCreateConnexionInterface()
{
	...
	CI->scxOpen=scxELISEEDOpen;
	CI->scxClose=scxELISEEDClose;
	CI->scxWrite=scxELISEEDWrite;
	CI->scxRead=scxELISEEDRead;
	CI->scxCfgAddress=scxELISEEDCfgAddress;
	CI->scxSetParameter=scxELISEEDSetParameter;
	CI->scxGetParameter=scxELISEEDGetParameter;
	CI->sciDestroyConnexionInterface=sciELISEEDDestroyConnexionInterface;
...
#else
#pragma SPGMSG(__FILE__,__LINE__,"NoELISED : Emule LISE ED disabled")
#endif
#endif
