...
namespace UnitySC.PM.ANA.Service.Shared.TestUtils
{
    ...
    {
        public AnaHardwareManager HardwareManager { get; set; }

        public CalibrationManager CalibManager { get; set; }
        ...
        public void Init()
        {
            ...
            if (this is ITestWithAxes testWithAxes)
                TestWithAxesHelper.Setup(testWithAxes);
            if (this is ITestWithLight testWithLight)
                TestWithLightHelper.Setup(testWithLight);
            if (this is ITestWithChuck testWithChuck)
                TestWithChuckHelper.Setup(testWithChuck);

            // testWithObjective must be done before testWithCamera and testWithProbeLise
            if (this is ITestWithObjective testWithObjective)
                TestWithObjectiveHelper.Setup(testWithObjective);
            if (this is ITestWithCamera testWithCamera)
                TestWithCameraHelper.Setup(testWithCamera);
            if (this is ITestWithProbeLise testWithProbeLise)
                TestWithProbeLiseHelper.Setup(testWithProbeLise);
