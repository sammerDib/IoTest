...
{
    public class AVTGE4900Camera : MatroxCameraBase
    {
        public override string SerialNumber { get => MdigInquireStringFeature("DeviceID"); }

        public AVTGE4900Camera(AVTGE4900CameraConfig config, IGlobalStatusServer globalStatusServer, ILogger logger) : base(config, globalStatusServer, logger)
        {
        }
        ...
        public override double GetFrameRate()
        {
            return Invoke(() =>
            {
                //double FrameRate = MdigInquireDoubleFeature("AcquisitionFrameRateAbs");
                double FrameRate = MdigInquireDoubleFeature("AcquisitionFrameRateLimit");
