...
//static CString strModule("FindSquare");
...
static H3_ARRAY_INT8 GetSgn(const H3_ARRAY2D_FLT32& ChessConvolI,const H3_ARRAY_PT2DFLT32& IntersecSE)
{
	...
	for(i=0;i<nb;i++)
	{
		Out[i]=(char)SGN(ChessConvolI((size_t)IntersecSE[i].y, (size_t)IntersecSE[i].x));
	}
	return Out;
}
...
template < class TYPE > static
bool ChessConvol2(CH3Array2D<TYPE>& Image,unsigned long N,H3_ARRAY2D_FLT32 &ChessConvol,	long PasUtile ,bool doSave=false)
{
	CString strFunction("ChessConvol2b");
	...
	if(ChessConvol.GetLi()!=ny || ChessConvol.GetCo()!=nx){
		ChessConvol.ReAlloc(ny,nx);
	}
	ChessConvol.Fill(0.0f);
	...
	float fSeuil=NN*CHESSCONVOL2_GL_SEUIL_NOIR_BLANC;
...
bool Convol(const H3_ARRAY2D& Image,const H3_ARRAY2D_FLT32& V,H3_ARRAY2D_FLT32& ConvolI)
{
	...
	if(&ConvolI==(H3_ARRAY2D_FLT32*)&Image){
		H3DebugError(strModule,strFunction,"les arguments 1 et 3 doivent etre distincts");
		return false;
	}
...
{
	unsigned long ny=Image.GetLi(),nx=Image.GetCo();
...
template < class TYPE > static
bool ReSample(const H3_ARRAY2D &Ima,unsigned long stepSE,H3_ARRAY2D& I_SE)
{
	CString strFunction("ReSample"),msg;
	msg.Format("stepSE=%d",stepSE);
	H3DebugInfo(strModule,strFunction,_T("FindSquare in ")+msg);

	if(&Ima==&I_SE){
		H3DebugInfo(strModule,strFunction,"FindSquare pb1");
		return false;
	}

	unsigned long nx=Ima.GetCo(),ny=Ima.GetLi();
	unsigned long i,j;

	//______________si stepSE>nx ou ny :PB
	unsigned long nxSE=nx/stepSE,nySE=ny/stepSE;
	H3DebugInfo(strModule,strFunction,"FindSquare 000");
	...
	H3DebugInfo(strModule,strFunction,"out true");
	return true;
}
...
template <class TYPE>
H3_ARRAY2D_PT2DFLT32 cornerfinder(const H3_ARRAY2D_PT2DFLT32& xt,
								...
								long  wx2,long  wy2,float fCornerFinderResolution,	long  nCornerFinderMaxIter)
{
	...
	H3_ARRAY2D_PT2DFLT32 xc(xt);//xc est initialisé avec xt
	...
	long crIx,crIy,crIx_old=-(long)nx,crIy_old=-(long)ny;//les coordonnées arrondies du i_ieme point
	float itIx,itIy;
	...
	for (i=0;i<nbPoints;i++){
		...
		{//Boucle1
			...
			crIx=(long)floor(cIx+0.5);
			crIy=(long)floor(cIy+0.5);
...
{
	CString strFunction("cornerfinder_firstGuess"),msg;
	H3DebugInfo("",strFunction,"in");
	const size_t nx=Image.GetCo(),ny=Image.GetLi();
	...
	const unsigned long N=nCornerFinderFirstGuessN;
	...
	H3DebugInfo("",strFunction,"2");
	//convolution par un damier
	H3_ARRAY2D_FLT32 ChessConvolI;
	if(!ChessConvol2(ImageSE,N,ChessConvolI,fCornerFinderSeuil,false)){
		return false;
	}
	...
	size_t nbPoints=IntersecSE.GetSize();

	if (nbPoints==0){
		H3DebugInfo(strModule,strFunction,"Le motif recherché n'a pas ete trouvé");
		return false;
	}
	...
	for(i=0,j=0;i<nbPoints;i++){
		x00=x0=(long)((IntersecSE[i].x+0.5)*stepSE)-d ;
		y00=y0=(long)((IntersecSE[i].y+0.5)*stepSE)-d ;

		if(x0<0){
			x0=0;
			dx=dx0/2+1;
		}
		else if(x0>(long)nx-dx0){
			if(x0>(long)nx-dx0/2) x0=nx-x0/2;
		...
		if(ChessConvol2(Imagette,N3,bwImagette,0,false)){
			...
			if(PointI.GetSize()>0){
				Sgn[j]=Sgn0[i];
				Intersec[j]=PointI[0]+H3_POINT2D_FLT32((float)x0,(float)y0);
				j++;
			}
			else{
	...
	H3DebugInfo("",strFunction,"cornerfinder_firstGuess 1");

	H3DebugInfo("",strFunction,"cornerfinder_firstGuess 2");

	CString s1;
	s1.Format("size=%d",Intersec.GetSize());

	Intersec.Resize(j);
	s1.Format("size=%d",Intersec.GetSize());
	
	Intersec.Resize(j);
	GR.Points.ReAlloc(j);for(i=0;i<j;i++)GR.Points[i]=Intersec[i];

	s1.Format("size=%d\tj=%d",GR.Points.GetSize(),j);
	
	s1.Format("Sgn:size=%d",Sgn.GetSize());
	...
	if(j>0){
		...
		GR.Points.ReAlloc(j);for(i=0;i<j;i++)GR.Points[i]=Intersec[i];
		Sgn.Resize(j);
		GR.Sgn.ReAlloc(j);for(i=0;i<j;i++)GR.Sgn[i]=Sgn[i];
	}
	H3DebugInfo("",strFunction,"cornerfinder_firstGuess out");
