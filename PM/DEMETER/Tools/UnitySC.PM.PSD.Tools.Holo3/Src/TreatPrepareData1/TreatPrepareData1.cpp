...
CTreatPrepareData1::CTreatPrepareData1()
{	
...
bool CTreatPrepareData1::Init(tmapTreatInitPrm& p_pPrmMap, const CString& calibFolder)
{ 	
	...
	else
	{
		...
		else
		{
			...
			while (oFile.ReadString(csLine))
			{
				...
				{
					m_vProlongeAreas.push_back(Rect(nX,	nY,	nW, nH)); 
				}
				else
				{ 
...
bool CTreatPrepareData1::Exec( const tmapTreatPrm& p_InputsPrm, tmapTreatPrm& p_OutputsPrm )
{
	...
	CWinThread* pThreadErode = AfxBeginThread(&CTreatPrepareData1::static_ErodeCV, this, THREAD_PRIORITY_NORMAL, 0, CREATE_SUSPENDED);
	...
	CWinThread* pThreadDilate = AfxBeginThread(&CTreatPrepareData1::static_DilateCV, this, THREAD_PRIORITY_NORMAL, 0, CREATE_SUSPENDED);
	...
	if(nSaveData != 0 && pSavData != nullptr)
	{
		...
		CWinThread* pThread = AfxBeginThread(&CTreatPrepareData1::SaveData, pSavData, THREAD_PRIORITY_NORMAL, 0, CREATE_SUSPENDED);
		if (pThread == 0)
		{
...
bool CTreatPrepareData1::FringeKillerNX( double p_dMin, int p_na, int p_nb1, int p_nb2, int p_nHThresh )
{
...
bool CTreatPrepareData1::FringeKillerNY( double p_dMin, int p_na1, int p_na2, int p_nb, int p_nHThresh )
{
	// using FFTW 3 Library
...
UINT CTreatPrepareData1::static_ErodeCV(void *p_pParameters)
{
	CTreatPrepareData1* pObj = static_cast<CTreatPrepareData1 *> (p_pParameters);
	pObj->ErodeCV();
	return 041;
}

void CTreatPrepareData1::ErodeCV()
{
...
UINT CTreatPrepareData1::static_DilateCV(void *p_pParameters)
{
	CTreatPrepareData1* pObj = static_cast<CTreatPrepareData1 *> (p_pParameters);
	pObj->DilateCV();
	return 042;
...
void CTreatPrepareData1::DilateCV()
{
...
bool CTreatPrepareData1::SaveGreyImageFlt32(CString p_csFilepath, shared_ptr<H3_MATRIX_FLT32> p_oMatrixFloat, float p_fMin /*= FLT_MAX*/, float p_fMax /*= FLT_MAX*/, bool bAutoscale /*= true*/)
{
...
bool CTreatPrepareData1::SaveGreyImageUInt8(CString p_csFilepath, shared_ptr<H3_MATRIX_UINT8> p_oMatrix, int p_nMin /*= INT_MAX*/, int p_nMax /*= INT_MAX*/, bool bAutoscale /*= true*/)
{
...
UINT CTreatPrepareData1::SaveData( void *p_pParameters )
{
...
HRESULT CTreatPrepareData1::QueryInterface( REFIID iid, void **ppvObject )
{
...
ULONG CTreatPrepareData1::AddRef( void )
{
	m_ulRefCount++;
	return m_ulRefCount;
}

ULONG CTreatPrepareData1::Release( void )
{
	m_ulRefCount--;
	if (m_ulRefCount!=0) 
		return m_ulRefCount;
...
bool CTreatPrepareData1::Prolonge_Bord()
{	
	...
	for (int i=0; i<m_vProlongeAreas.size(); i++)
	{
		...
		pThreadPtrArray[i] = AfxBeginThread(&CTreatPrepareData1::static_ProlongeBordVignette, pData, THREAD_PRIORITY_NORMAL, 0, CREATE_SUSPENDED);
...
UINT CTreatPrepareData1::static_ProlongeBordVignette( void *p_pParameters )
{
	tProlongData* pData			= static_cast<tProlongData *> (p_pParameters); 
	CTreatPrepareData1* pObj	= static_cast<CTreatPrepareData1 *> (pData->_TreatPtr);
...
bool CTreatPrepareData1::ProlongeBordVignette(cv::Mat& p_SubDist, cv::Mat& p_SubNx, cv::Mat& p_SubNy, cv::Mat& p_oSubMask, cv::Mat& p_oSubMaskInv, HANDLE p_hEvenDone)
{
...
{
	CTreatPrepareData1 *pObj = new CTreatPrepareData1();
	if (pObj==0) 
