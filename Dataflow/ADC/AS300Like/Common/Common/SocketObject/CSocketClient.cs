...
{    
    ...
    public delegate void OnClientDisconnected(enumConnection ServerType);
    public enum enumALTAStatus
    {
    ...
    public class CSocketClient<T> where T : Exception
    {
        ...
        public CSocketClient(enumConnection pServerType, String ServerName, short PortNum, OnSocketLog pEventLog, OnServerConnectedDisconnected pOnServerDisconnected)
        {
        ...
        public Boolean IsConnected { get { 
            return (m_SocketClient!=null && m_SocketClient.IsConnected) ; 
        } }
        ...
        {             
            ...
            while (!m_evtDataReceivedSending.WaitOne(100, false))
            {
                ...
                bStopTimeOut = (((TimeSpan)DateTime.Now.Subtract(StartSend)).Seconds > TIMEOUT_SENDMSG);
        ...
        //-------------------------------------------------------------------------------------------------------------------------------------------------------       
        public int GetGrabberStatus(FctCode Fct, out enumALTAStatus Status)
        {
            String sReponse;
            short iStatus = 0;
            Status = enumALTAStatus.ALTAMSG_STATUS_0_NONE;  
            int ErrorCode = SendMsg(Fct, "", out sReponse);
            if ((ErrorCode == 0) || (sReponse!="")) 
            {
                // Take Status from String
                String[] sTab = sReponse.Split('-');
                iStatus = Convert.ToInt16(sTab[0].Trim()); 
                // Check status conformity
                if ((iStatus > (int)enumALTAStatus.ALTAMSG_STATUS_0_NONE) && (iStatus < (int)enumALTAStatus.ALTAMSG_STATUS_SAVING_ERROR))
                {
                    Status = (enumALTAStatus)iStatus;
                    return ETCPException.NOERROR;
                }
                else
        ...
        //-------------------------------------------------------------------------------------------------------------------------------------------------------       
        public int SendInitGrabber(String sParameters, out String sReponse)
        {
        ...
        public void Disconnect()
        {
            try
            {
                if (m_SocketClient != null)
                {
                    ...
                    throw new ETCPException( m_ServerType ,ETCPException.NO_ERROR_ALREADY_DISCONNECTED, "Already disconnected.", "in fct Disconnect()", false);
            }
            catch (ETCPException Ex)
            {
                Ex.DisplayError(Ex.ErrorCode);
