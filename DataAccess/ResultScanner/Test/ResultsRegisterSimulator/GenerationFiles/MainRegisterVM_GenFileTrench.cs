...
namespace ResultsRegisterSimulator
{
    ...
    {
        #region Generate Trench files
        private static void GenerateTRENCHSampleFiles(ref BackgroundWorker worker, ref int total, ref int complete)
        {
            ...
            var ToleranceSet = new TrenchResultSettings()
            {
                DepthTolerance = new LengthTolerance(5, LengthToleranceUnit.Micrometer),
                DepthTarget = new Length(110, LengthUnit.Micrometer),
                WidthTolerance = new LengthTolerance(10, LengthToleranceUnit.Micrometer),
                WidthTarget = new Length(500, LengthUnit.Micrometer),
            };
            var AcqSimSet = new TrenchResultSettings()
            {
                DepthTolerance = new LengthTolerance(8, LengthToleranceUnit.Micrometer),
                DepthTarget = new Length(111, LengthUnit.Micrometer),
                WidthTolerance = new LengthTolerance(10, LengthToleranceUnit.Micrometer),
                WidthTarget = new Length(495, LengthUnit.Micrometer),
            };

            double[] stateproba = new double[] { 0.8, 0.15, 0.05, 0.0 };
            System.Diagnostics.Debug.Assert(stateproba.Sum() == 1.0);
            // slot 0 
            GenerateTRENCHSampleFile(ref worker, ref total, ref complete, slot++, 200, 1, xs_10pts, ys_10pts, ToleranceSet, AcqSimSet, stateproba, 0);

            // slot 1 
            AcqSimSet.DepthTolerance.Value = 5;
            AcqSimSet.DepthTarget = ToleranceSet.DepthTarget;
            GenerateTRENCHSampleFile(ref worker, ref total, ref complete, slot++, 200, 3, xs_10pts, ys_10pts, ToleranceSet, AcqSimSet, stateproba, 50);

            // slot 2  
            AcqSimSet.WidthTolerance.Value = 12;
            stateproba = new double[] { 0.65, 0.19, 0.15, 0.01 };
            System.Diagnostics.Debug.Assert(stateproba.Sum() == 1.0);
            GenerateTRENCHSampleFile(ref worker, ref total, ref complete, slot++, 200, 5, xs_10pts, ys_10pts, ToleranceSet, AcqSimSet, stateproba, 50);

            // slot 3 
            var ToleranceSet2 = new TrenchResultSettings()
            {
                DepthTolerance = new LengthTolerance(90, LengthToleranceUnit.Nanometer),
                DepthTarget = new Length(850, LengthUnit.Nanometer),
                WidthTolerance = new LengthTolerance(3, LengthToleranceUnit.Micrometer),
                WidthTarget = new Length(25, LengthUnit.Micrometer)
            };

            var AcqSimSet2 = new TrenchResultSettings()
            {
                DepthTolerance = new LengthTolerance(93, LengthToleranceUnit.Nanometer),
                DepthTarget = new Length(840, LengthUnit.Nanometer),
                WidthTolerance = new LengthTolerance(4, LengthToleranceUnit.Micrometer),
                WidthTarget = new Length(23, LengthUnit.Micrometer)
            };
            stateproba = new double[] { 0.9, 0.09, 0.01, 0.0 };
            System.Diagnostics.Debug.Assert(stateproba.Sum() == 1.0);
            GenerateTRENCHSampleFile(ref worker, ref total, ref complete, slot++, 300, 1, xs_30pts, ys_30pts, ToleranceSet2, AcqSimSet2, stateproba, 0);

            // slot 4 
            stateproba = new double[] { 0.4, 0.35, 0.25, 0.0 };
            System.Diagnostics.Debug.Assert(stateproba.Sum() == 1.0);
            AcqSimSet2.DepthTolerance.Value = 200;
            GenerateTRENCHSampleFile(ref worker, ref total, ref complete, slot++, 300, 4, xs_30pts, ys_30pts, ToleranceSet2, AcqSimSet2, stateproba, 120);
    

            //Slot 5 - Die
            var ToleranceSet4 = new TrenchResultSettings()
            {
                DepthTolerance = new LengthTolerance(3, LengthToleranceUnit.Micrometer),
                DepthTarget = new Length(55, LengthUnit.Micrometer),
                WidthTolerance = new LengthTolerance(5, LengthToleranceUnit.Micrometer),
                WidthTarget = new Length(100, LengthUnit.Micrometer)
            };
            var AcqSimSet4 = new TrenchResultSettings()
            {
                DepthTolerance = new LengthTolerance(3, LengthToleranceUnit.Micrometer),
                DepthTarget = new Length(56, LengthUnit.Micrometer),
                WidthTolerance = new LengthTolerance(5, LengthToleranceUnit.Micrometer),
                WidthTarget = new Length(102, LengthUnit.Micrometer)
            };
            ...
            GenerateTRENCHDieSampleFile(ref worker, ref total, ref complete, slot++, 200, 80, 3, DieSizW_mm, DieSizH_mm, 15.0,
                 xdies_4pts, ydies_4pts, ToleranceSet4, AcqSimSet4, stateproba, true, 0);

            //Slot 6 - Die - 
            xdies_4pts = new double[] { 0.5, 0.7, 0.65, 1.4 };
            ydies_4pts = new double[] { 0.2, 0.3, 0.5, 1.0 };
            GenerateTRENCHDieSampleFile(ref worker, ref total, ref complete, slot++, 200, 60, 1, DieSizW_mm, DieSizH_mm, 20.0,
                 xdies_4pts, ydies_4pts, ToleranceSet4, AcqSimSet4, stateproba, true, 37);
            ...
            GenerateTRENCHDieSampleFile(ref worker, ref total, ref complete, slot++, 150, 75, 5, DieSizW_mm, DieSizH_mm, 0.034,
                xdies_2pts, ydies_2pts, ToleranceSet4, AcqSimSet4, stateproba, true, 0);

        }

        private static RawProfile GenerateTRENCHRawProfile(TrenchResultSettings acqSettings, Length scanLength, Length stepHeight, int nbPoint, double rndStartPos = 0.01)
        {
            var rnd = new Random();
            ...
            double sigmaZ = acqSettings.DepthTolerance.GetAbsoluteTolerance(acqSettings.DepthTarget).GetValueAs(stepHeight.Unit);
        ...
        private static void GenerateTRENCHSampleFile(ref BackgroundWorker worker, ref int total, ref int complete, int slot, int waferdiameter_mm, int nbRepeta, double[] xs, double[] ys, TrenchResultSettings toleranceSettings, TrenchResultSettings acqSettings, double[] stateProba, int useScanRawProfilePts = 0)
        {
            ...
            var res = new MetroResult(ResultType.ANALYSE_Trench);
            res.ResFilePath = Path.Combine(GeneratePath, $"S{slot}.{ResultType.ANALYSE_Trench.GetExt()}");

            var MeasuresSimulated = new TrenchResult();
            ...
            MeasuresSimulated.Name = $"S{slot} simulated TRENCH result";
            MeasuresSimulated.Information = $"Simulated TRENCH resuls, NbSite = {xs.Length}, NbRepeta = {nbRepeta}";
            ...
            {
                var ptRes = new TrenchPointResult();
                ...
                {
                    var acqdata = new TrenchPointData();
                    ...
                    if (acqdata.State != MeasureState.NotMeasured)
                    {
                        ...
                        acqdata.Depth = new Length(NextGaussian(rnd, acqSettings.DepthTarget.Value, 3.0 * acqSettings.DepthTolerance.Value), acqSettings.DepthTarget.Unit);
                        ...
                        if (useScanRawProfilePts != 0)
                            acqdata.RawProfileScan = GenerateTRENCHRawProfile(acqSettings, acqdata.Width, acqdata.Depth, useScanRawProfilePts);
                    }
                    else
            ...
            if (!res.WriteInFile(res.ResFilePath, out string serrror))
                MessageBox.Show($"Cannot generate S{slot}.anatrench :\n{serrror}");
            else
            {
        ...
        private static void GenerateTRENCHDieSampleFile(ref BackgroundWorker worker, ref int total, ref int complete, int slot,
           ...
           TrenchResultSettings toleranceSettings, TrenchResultSettings acqSettings, double[] stateProba, bool computeDieGridXY = false, int useScanRawProfilePts = 0)
        {
            double probsum = stateProba.Sum();
            if ((probsum - 1.0) > 0.00001)
            {
                MessageBox.Show($"S{slot} anatch proba sum state is not equal to 1.0");
                return;
            }
            ...
            if (computeDieGridXY)
            {
                ...
                DM.DieReferenceColumnIndex = 0;
                DM.DieReferenceRowIndex = nDieRowMax;
            }
            ...
            MeasuresSimulated.Name = $"S{slot} simulated TRENCH DIE result";
            MeasuresSimulated.Information = $"Simulated TRENCH DIE resuls, NbDies = {NbDiesPresent} NbSite = {xdies.Length}, NbRepeta = {nbRepeta}";
            MeasuresSimulated.Settings = toleranceSettings;
        ...
        #endregion //Generate TRENCH files
