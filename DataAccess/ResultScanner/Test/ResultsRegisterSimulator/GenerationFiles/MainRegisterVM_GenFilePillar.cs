...
namespace ResultsRegisterSimulator
{
    ...
    {
        #region Generate Pillar files
        private static void GeneratePillarSampleFiles(ref BackgroundWorker worker, ref int total, ref int complete)
        {
            ...
            double[] stateproba = new double[] { 0.75, 0.19, 0.05, 0.01 };
            ...
            for (int n = 0; n < nbfile; n++)
            {
                ...
                double h = (300.0 - 20.0) * rnd.NextDouble() + 20.0;
                double w = (500.0 - 50.0) * rnd.NextDouble() + 50.0;
                double he = (10.0 - -10.0) * rnd.NextDouble() - 10.0;
                double we = (20.0 - -20.0) * rnd.NextDouble() - 20.0;

                // success , partial, error, notmeasure
                var ToleranceSet = new PillarResultSettings()
                {
                ...
                var AcqSimSet = new PillarResultSettings()
                {
                ...
                GeneratePillarSampleFile(ref worker, ref total, ref complete, slot++, 200, repeta, xs, ys, ToleranceSet, AcqSimSet,  stateproba, rnd, userawprofiles);
            }
        }

        private static RawProfile GeneratePillarRawProfile(PillarResultSettings acqSettings, Length scanLength, Length stepHeight, int nbPoint, Random rnd, double rndStartPos = 0.01)
        {
        ...
        private static void GeneratePillarSampleFile(ref BackgroundWorker worker, ref int total, ref int complete, int slot, int waferdiameter_mm, int nbRepeta, double[] xs, double[] ys, PillarResultSettings toleranceSettings, PillarResultSettings acqSettings, double[] stateProba, Random rnd, int useScanRawProfilePts = 0)
        {
            ...
            MeasuresSimulated.Name = $"S{slot} simulated PILLAR result";
            MeasuresSimulated.Information = $"Simulated PILLAR resuls, NbSite = {xs.Length}, NbRepeta = {nbRepeta}";
            ...
            {
                var ptRes = new PillarPointResult();
                ...
                {
                    var acqdata = new PillarPointData();
                    ...
                    if (acqdata.State != MeasureState.NotMeasured)
                    {
                        ...
                        if (useScanRawProfilePts != 0)
                            acqdata.RawProfileScan = GeneratePillarRawProfile(acqSettings, acqdata.Width, acqdata.Height, useScanRawProfilePts, rnd);
                    }
                    else
            ...
            if (!res.WriteInFile(res.ResFilePath, out string serrror))
                MessageBox.Show($"Cannot generate S{slot}.anapil :\n{serrror}");
            else
            {
        ...
        #endregion //Generate pillar files
