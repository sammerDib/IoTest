...
namespace ResultsRegisterSimulator
{
    ...
    {
        #region Generate Topo files
        private static void GenerateTOPOSampleFiles(ref BackgroundWorker worker, ref int total, ref int complete)
        {
            ...
            { Name = "Out 1", OutputTarget = 30.0, OutputTolerance = new Tolerance(1.5, ToleranceUnit.AbsoluteValue) };
            var otpB = new ExternalProcessingOutput()
            { Name = "Output 2", OutputTarget = 0.375, OutputTolerance = new Tolerance(0.015, ToleranceUnit.AbsoluteValue) };
            var otpC = new ExternalProcessingOutput()
            { Name = "Oo 3", OutputTarget = 50.56, OutputTolerance = new Tolerance(2.5, ToleranceUnit.Percentage) };
            var otpD = new ExternalProcessingOutput()
            { Name = "LongOutputName4", OutputTarget = 9500.0, OutputTolerance = new Tolerance(2.0, ToleranceUnit.Percentage) };


            int slot = 0;
            // success , partial, error, notmeasure
            var ToleranceSet = new TopographyResultSettings()
            {
                ExternalProcessingOutputs = new List<ExternalProcessingOutput>() { otpA, otpB, otpC, otpD }
            };
            var AcqSimSet = new TopographyResultSettings()
            {
                ExternalProcessingOutputs = new List<ExternalProcessingOutput>() { otpA, otpB, otpC, otpD }
            };
            ...
            // slot 0 
            GenerateTOPOSampleFile(ref worker, ref total, ref complete, slot++, 200, 1, xs_10pts, ys_10pts, ToleranceSet, AcqSimSet, stateproba);
            ...
            GenerateTOPOSampleFile(ref worker, ref total, ref complete, slot++, 200, 3, xs_10pts, ys_10pts, ToleranceSet, AcqSimSet, stateproba);
            ...
            System.Diagnostics.Debug.Assert(stateproba.Sum() == 1.0);
            GenerateTOPOSampleFile(ref worker, ref total, ref complete, slot++, 200, 5, xs_10pts, ys_10pts, ToleranceSet, AcqSimSet, stateproba);

            // slot 3 
            var ToleranceSet2 = new TopographyResultSettings()
            {
                ExternalProcessingOutputs = new List<ExternalProcessingOutput>() { otpD }
            };

            var AcqSimSet2 = new TopographyResultSettings()
            {
                ExternalProcessingOutputs = new List<ExternalProcessingOutput>() { otpD }
            };
            stateproba = new double[] { 0.9, 0.09, 0.01, 0.0 };
            System.Diagnostics.Debug.Assert(stateproba.Sum() == 1.0);
            GenerateTOPOSampleFile(ref worker, ref total, ref complete, slot++, 150, 1, xs_20pts, ys_20pts, ToleranceSet2, AcqSimSet2, stateproba);

            // slot 4 
            stateproba = new double[] { 0.4, 0.35, 0.25, 0.0 };
            System.Diagnostics.Debug.Assert(stateproba.Sum() == 1.0);
            GenerateTOPOSampleFile(ref worker, ref total, ref complete, slot++, 150, 4, xs_20pts, ys_20pts, ToleranceSet2, AcqSimSet2, stateproba);

            // slot 5 
            AcqSimSet2.ExternalProcessingOutputs = null;
            stateproba = new double[] { 0.1, 0.0, 0.5, 0.4 };
            System.Diagnostics.Debug.Assert(stateproba.Sum() == 1.0);
            GenerateTOPOSampleFile(ref worker, ref total, ref complete, slot++, 150, 2, xs_20pts, ys_20pts, ToleranceSet2, AcqSimSet2, stateproba);

            // slot 6 
            var ToleranceSet3 = new TopographyResultSettings()
            {
                ExternalProcessingOutputs = new List<ExternalProcessingOutput>() { otpA, otpB }
            };
            var AcqSimSet3 = new TopographyResultSettings()
            {
                ExternalProcessingOutputs = new List<ExternalProcessingOutput>() { otpA, otpB }
            };
            stateproba = new double[] { 0.75, 0.2, 0.05, 0.0 };
            System.Diagnostics.Debug.Assert(stateproba.Sum() == 1.0);
            GenerateTOPOSampleFile(ref worker, ref total, ref complete, slot++, 200, 3, xs_5pts, ys_5pts, ToleranceSet3, AcqSimSet3, stateproba);

            //Slot 7 - Die
            var ToleranceSet4 = new TopographyResultSettings()
            {
                ExternalProcessingOutputs = new List<ExternalProcessingOutput>() { otpA, otpB }
            };
            var AcqSimSet4 = new TopographyResultSettings()
            {
                ExternalProcessingOutputs = new List<ExternalProcessingOutput>() { otpA, otpB }
            };
            ...
            GenerateTOPODieSampleFile(ref worker, ref total, ref complete, slot++, 150, 60, 3, DieSizW_mm, DieSizH_mm, 15.0,
            ...
            GenerateTOPODieSampleFile(ref worker, ref total, ref complete, slot++, 150, 60, 1, DieSizW_mm, DieSizH_mm, 20.0,
                 xdies_4pts, ydies_4pts, ToleranceSet4, AcqSimSet4, stateproba, false, true);

            //Slot 9 - Big Die - thumbnails 3D
            DieSizW_mm = 21.5;
            DieSizH_mm = 25.3;
            double[] xdies_2pts = new double[] { 0.45 * DieSizW_mm, 0.35 * DieSizW_mm, };
            double[] ydies_2pts = new double[] { 0.85 * DieSizH_mm, 0.25 * DieSizH_mm, };
            GenerateTOPODieSampleFile(ref worker, ref total, ref complete, slot++, 150, 75, 5, DieSizW_mm, DieSizH_mm, 0.034,
                xdies_2pts, ydies_2pts, ToleranceSet4, AcqSimSet4, stateproba, true, true);

            //Slot 10 - Big Die - numerous repeta No thumbs
            GenerateTOPODieSampleFile(ref worker, ref total, ref complete, slot++, 150, 75, 20, DieSizW_mm, DieSizH_mm, 0.034,
              xdies_2pts, ydies_2pts, ToleranceSet4, AcqSimSet4, stateproba, false, true);

        }

        private static void GenerateTOPOSampleFile(ref BackgroundWorker worker, ref int total, ref int complete, int slot, int waferdiameter_mm, int nbRepeta, double[] xs, double[] ys, TopographyResultSettings toleranceSettings, TopographyResultSettings acqSettings, double[] stateProba)
        {
            ...
            var res = new MetroResult(ResultType.ANALYSE_Topography);
            res.ResFilePath = Path.Combine(GeneratePath, $"S{slot}.{ResultType.ANALYSE_Topography.GetExt()}");

            var MeasuresSimulated = new TopographyResult();
            ...
            MeasuresSimulated.Name = $"S{slot} simulated TOPO result";
            MeasuresSimulated.Information = $"Simulated TOPO resuls, NbSite = {xs.Length}, NbRepeta = {nbRepeta}";
            ...
            {
                var ptRes = new TopographyPointResult();
                ...
                {
                    var acqdata = new TopographyPointData();
        ...
        private static void GenerateTOPODieSampleFile(ref BackgroundWorker worker, ref int total, ref int complete, int slot,
            ...
            TopographyResultSettings toleranceSettings, TopographyResultSettings acqSettings, double[] stateProba, bool genThumbnails = false, bool computeDieGridXY = false)
        {
            double probsum = stateProba.Sum();
            ...
            if (genThumbnails)
            {
                NtpThumb3DPath = Path.Combine(GeneratePath, $"S{slot}_TOPO_Thumbs");
            ...
            if (computeDieGridXY)
            {
                ...
                DM.DieReferenceColumnIndex = nDieColMax / 4;
                DM.DieReferenceRowIndex = nDieRowMax / 4;
            }
            ...
            MeasuresSimulated.Name = $"S{slot} simulated TOPO DIE result";
            MeasuresSimulated.Information = $"Simulated TOPO DIE resuls, NbDies = {NbDiesPresent} NbSite = {xdies.Length}, NbRepeta = {nbRepeta}";
            MeasuresSimulated.Settings = toleranceSettings;
                            ...
                            for (int k = 0; k < nbRepeta; k++)
                            {
                                ...
                                if (acqdata.State != MeasureState.NotMeasured)
                                {
                                    ...
                                    if (genThumbnails)
                                    {
                                        ...
                                        acqdata.ResultImageFileName = Path.Combine($".\\S{slot}_TOPO_Thumbs", $"{i}_{j}_{nx}_{k}.3da");
                                    }
                                }
                                ptRes.Datas.Add(acqdata);
                            }
