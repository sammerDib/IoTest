...
namespace UnitySC.GUI.Common.Vendor.Views.Panels.Setup.AccessRights
{
    ...
    public class AccessRightsPanel : SetupPanel
    {
        ...
        {
            LocalizationManager.AddLocalizationProvider(new ResourceFileProvider(typeof(AccessRightsResources)));
            LocalizationManager.AddLocalizationProvider(new ResourceFileProvider(typeof(EnumLocalization.AccessLevel)));
        }
            ...
            : base(id, icon)
        {
            ...
            NavigationTreeSource.Filter.Add(new FilterSwitch<TreeNode<MenuItemRightsViewModel>>(nameof(AccessRightsResources.ACCESSRIGHTSPANEL_MODIFIED), node => node.Model.HasModified));
            NavigationTreeSource.Filter.AddEnumFilter(nameof(AccessRightsResources.ACCESSRIGHTSPANEL_ACCESSIBILITY), node => node.Model.EnabledRight.Level);
            NavigationTreeSource.Filter.AddEnumFilter(nameof(AccessRightsResources.ACCESSRIGHTSPANEL_VISIBILITY), node => node.Model.VisibilityRight.Level);
            NavigationTreeSource.Search.AddSearchDefinition(nameof(AccessRightsResources.ACCESSRIGHTSPANEL_NAME), model => model.Element.LocalizedName, true);
            NavigationTreeSource.Search.AddSearchDefinition(nameof(AccessRightsResources.ACCESSRIGHTSPANEL_ID), model => model.Element.RelativeId, true);
            NavigationTreeSource.Sort.AddSortDefinition(new LocalizableText(nameof(AccessRightsResources.ACCESSRIGHTSPANEL_ORDER)), _ => null);
            NavigationTreeSource.Sort.AddSortDefinition(new LocalizableText(nameof(AccessRightsResources.ACCESSRIGHTSPANEL_LEVEL)), node => node.Model.HighestLevel);
        ...
        private void OnLevelChanged(RightViewModel obj)
        {
            ...
            if (NavigationTreeSource.Filter.IsApplied
                ...
                != nameof(AccessRightsResources.ACCESSRIGHTSPANEL_NAME))
            {
                NavigationTreeSource.RefreshVisibleTreeNodes();
            }
        }
