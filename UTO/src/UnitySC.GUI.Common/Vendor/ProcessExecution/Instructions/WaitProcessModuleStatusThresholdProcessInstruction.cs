...
{
    public class WaitProcessModuleStatusThresholdProcessInstruction
        : ProcessModuleInstruction<ProcessModule>, IInstructionAbortable, ISkippableInstruction
    {
        #region Declaration

        private readonly WaitProcessModuleStatusThresholdInstruction _waitProcessModuleStatusThresholdInstruction;
        ...
        public WaitProcessModuleStatusThresholdProcessInstruction(
            Agileo.EquipmentModeling.Equipment equipment,
            WaitProcessModuleStatusThresholdInstruction recipeInstructions)
        {
            _equipment = equipment;
            _waitProcessModuleStatusThresholdInstruction = recipeInstructions;
            _logger = Agileo.Common.Logging.Logger.GetLogger(
                nameof(WaitProcessModuleStatusThresholdProcessInstruction));
            InstructionType = InstructionType.WaitInstruction;
        }
        ...
        {
            var lstStatus = _waitProcessModuleStatusThresholdInstruction.Thresholds.OfType<ThresholdOperand>()
                .Where(y => y.StatusName == e.Status.Name)
                .ToList();
        ...
        private bool ControlOperation()
        {
            ...
            foreach (var c in _waitProcessModuleStatusThresholdInstruction.Thresholds.OfType<ThresholdOperator>())
            {
                opIndex++;
        ...
        private bool ControlQuantity(ThresholdOperand threshold)
        {
            var device = _equipment.GetTopDeviceContainer()
                .AllDevices()
                .FirstOrDefault(x => x.Name == ProcessModule.Name);
            switch (threshold)
            {
        ...
        {
            foreach (var th in _waitProcessModuleStatusThresholdInstruction.Thresholds.OfType<ThresholdOperand>())
            {
                var dev = _equipment.AllDevices().FirstOrDefault(x => x.Name == ProcessModule.Name);
                if (dev != null)
                {
        ...
        public override void Execute()
        {
            ...
            foreach (var th in _waitProcessModuleStatusThresholdInstruction.Thresholds.OfType<ThresholdOperand>())
            {
                var processModule = _equipment.AllDevices<ProcessModule>()
                    .FirstOrDefault(x => x.Name == ProcessModule.Name);

                if (processModule != null)
                {
                    processModule.StatusValueChanged += Dev_StatusValueChanged;
                    foreach (var dev in processModule.AllDevices().Where(d => d != null))
                    {
                        dev.StatusValueChanged += Dev_StatusValueChanged;
                    }
                }
            ...
            if (!ControlOperation())
            {
                ...
                var timeOut = _waitProcessModuleStatusThresholdInstruction.TimeOut.Milliseconds;
                var wea = new WaitEndOfContractHelper();
                wea.WaitEndOfContract(
                    "WaitProcessModuleStatusThreshold",
                    Duration.From(
                        _waitProcessModuleStatusThresholdInstruction.IsTimeoutEnabled ? timeOut : Timeout.Infinite,
        ...
        protected override Instruction CloneInstruction()
            => new WaitProcessModuleStatusThresholdProcessInstruction(
                _equipment,
                _waitProcessModuleStatusThresholdInstruction)
            {
        ...
        protected override void LocalizeName()
            => Name = LocalizationManager.GetString(
                nameof(ProcessInstructionsResources.WAIT_PM_STATUS_THRESHOLD_INSTRUCTION));
        ...
        public void Skip() => _tcs?.TrySetResult(TaskCompletionInfos.Stopped);
