...
namespace UnitySC.GUI.Common.Vendor.UIComponents.Components.DataTree.Controls
{
    public class DataTreeItem : Control
    {
        ...
        public override void OnApplyTemplate()
        {
            ...
            if (GetTemplateChild(ElementHeaderGrid) is UIElement headerGrid)
            {
                headerGrid.MouseEnter += HeaderGrid_MouseEnter;
                headerGrid.MouseLeave += HeaderGrid_OnMouseLeave;
                headerGrid.MouseDown += HeaderGrid_MouseDown;
                headerGrid.MouseUp += HeaderGrid_OnMouseUp;
        ...
        {
            if (e.Handled) return;
            if (DataContext is not ITreeNode treeNode) return;
        ...
        {
            if (d is DataTreeItem self && !self.IsClicked)
            {
                self._clickedPoint = null;
            }
        }
