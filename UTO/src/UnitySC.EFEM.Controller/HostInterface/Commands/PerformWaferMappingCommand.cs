...
{
    /// <summary>
    /// Class responsible to handle the perform mapping command defined in
    /// EFEM Controller Comm Specs 211006.pdf §4.3.3 oWMAP
    /// </summary>
    public class PerformWaferMappingCommand : LoadPortCommand
    {
        public PerformWaferMappingCommand(
            Constants.Port loadPortId,
            IMacroCommandSender sender,
            IEquipmentFacade eqFacade,
            ILogger logger,
            EfemEquipmentManager equipmentManager)
            : base(Constants.Commands.PerformWaferMapping, loadPortId, sender, eqFacade, logger, equipmentManager)
        {
        }
        ...
        protected override bool TreatOrder(Message message)
        {
            ...
            if (!LoadPort.CanExecute(nameof(ILoadPort.Map), out CommandContext context))
            {
                ...
                foreach (var error in context?.Errors ?? new List<string>())
                {
                    ...
                    if (CancelIfIsInServiceFailed(message, error)
                        ...
                        || CancelIfIsMappingSupportedFailed(message, error))
                    {
                        return true;
                    }
            ...
            LoadPort.MapAsync().ContinueWith(antecedent => SendCommandResult(antecedent, deviceId));
