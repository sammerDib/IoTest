...
{
    /// <summary>
    /// Class responsible to handle the set carrier in access command defined in
    /// EFEM Controller Comm Specs 211006.pdf §4.3.1 oDOCK
    /// </summary>
    public class DockCommand : LoadPortCommand
    {
        public DockCommand(
            ...
            : base(Constants.Commands.Dock, loadPortId, sender, eqFacade, logger, equipmentManager)
        {
        }
        ...
        protected override bool TreatOrder(Message message)
        {
            ...
            if (LoadPort.CarrierTypeName == LoadPort.UnknownCarrierName)
            {
                SendCancellation(message, Constants.Errors[ErrorCode.LoadPortCarrierTypeUndefined]);
                return true;
            }
            ...
            if (!LoadPort.CanExecute(nameof(ILoadPort.Open), out CommandContext context, true))
            {
            ...
            LoadPort.OpenAsync(true).ContinueWith(antecedent => SendCommandResult(antecedent, deviceId));
