...
{
    /// <summary>
    /// Class responsible to handle the close loadport door command defined in
    /// EFEM Controller Comm Specs 211006.pdf §4.3.5 oLPCL
    /// </summary>
    public class CloseDoorCommand : LoadPortCommand
    {
        public CloseDoorCommand(
            ...
            : base(Constants.Commands.CloseDoor, loadPortId, sender, eqFacade, logger, equipmentManager)
        {
        }
        ...
        protected override bool TreatOrder(Message message)
        {
            ...
            if (!LoadPort.CanExecute(nameof(ILoadPort.Close), out CommandContext context))
            {
            ...
            LoadPort.CloseAsync().ContinueWith(antecedent => SendCommandResult(antecedent, deviceId));
