...
{
    public class CheckRobotArmEnabled : CSharpCommandConditionBehavior
    {
        public override void Check(CommandContext context)
        {
            ...
            {
                case RobotArm.Arm1:
                    if (!robot.Configuration.UpperArm.IsEnabled)
                    {
                        context.AddContextError(Messages.RobotArm1NotEnabled);
                    }

                    break;
                case RobotArm.Arm2:
                    if (!robot.Configuration.LowerArm.IsEnabled)
                    {
                        context.AddContextError(Messages.RobotArm2NotEnabled);
                    }
