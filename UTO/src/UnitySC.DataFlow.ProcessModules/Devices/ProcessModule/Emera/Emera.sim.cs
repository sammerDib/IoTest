    ...
    public partial class Emera
    {
        public override List<StatusVariable> SimulateGetStatusVariables(List<int> ids)
        {
            ...
            if (ids.Count == 0)
            {
                statusVariables.Add(
                    new StatusVariable(
                        nameof(SVName_EME.State),
                        VidDataType.String,
                        "Get PM state to Toolcommander")
                    { Value = "OffLine" });
                statusVariables.Add(
                    new StatusVariable(
                        nameof(SVName_EME.RecipeName),
                        VidDataType.String,
                        "Get selected Recipe name to Toolcommander")
                    { Value = "JobTest" });
                statusVariables.Add(
                    new StatusVariable(
                        nameof(SVName_EME.ReadyForProcess),
                        VidDataType.Boolean,
                        "Get state if PM is ready for for Processing")
                    { Value = true });
                statusVariables.Add(
                    new StatusVariable(
                        nameof(SVName_EME.RecipeActive),
                        VidDataType.Boolean,
                        "Get state if recipe is currently running")
                    { Value = false });
                ...
                switch (id)
                {
                    case (int)SVName_EME.State:
                        statusVariables.Add(
                            ...
                            { Value = "OffLine" });
                        break;
                    case (int)SVName_EME.RecipeName:
                        statusVariables.Add(
                            new StatusVariable(
                                nameof(SVName_EME.RecipeName),
                                VidDataType.String,
                            ...
                            { Value = "JobTest" });
                        break;
                    case (int)SVName_EME.ReadyForProcess:
                        statusVariables.Add(
                            new StatusVariable(
                                nameof(SVName_EME.ReadyForProcess),
                                VidDataType.Boolean,
                            ...
                            { Value = true });
                        break;
                    case (int)SVName_EME.RecipeActive:
                        statusVariables.Add(
                            new StatusVariable(
                                nameof(SVName_EME.RecipeActive),
                                VidDataType.Boolean,
