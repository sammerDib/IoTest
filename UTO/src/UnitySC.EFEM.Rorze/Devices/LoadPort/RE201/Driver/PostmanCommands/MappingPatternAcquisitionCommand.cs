...
using UnitySC.EFEM.Rorze.Devices.LoadPort.RE201.Driver.EventArgs;
using UnitySC.EFEM.Rorze.Drivers;

namespace UnitySC.EFEM.Rorze.Devices.LoadPort.RE201.Driver.PostmanCommands
{
    public class MappingPatternAcquisitionCommand : RE201Command
    {
        #region RorzeCommand
        ...
        protected override bool TreatAck(RorzeMessage message)
        {
            ...
            var slotStates = new List<RE201SlotState>(mappingPattern.Length);
            slotStates.AddRange(mappingPattern.Select(t =>
                (RE201SlotState)Enum.Parse(typeof(RE201SlotState), t.ToString())));

            var mappingArgs = new MappingEventArgs(new Collection<RE201SlotState>(slotStates));
        ...
        {
            return new MappingPatternAcquisitionCommand(RorzeConstants.CommandTypeAbb.Order, deviceId, sender,
                eqFacade);
        }
        ...
        private MappingPatternAcquisitionCommand(
            ...
            : base(commandType, RorzeConstants.DeviceTypeAbb.LoadPort, deviceId,
                RorzeConstants.Commands.MappingPatternAcquisition, sender, eqFacade)
        {
        }
