...
{
    public class OriginSearchCommand : RV201Command
    {
        #region Constructors

        public static OriginSearchCommand NewOrder(
            byte deviceId,
            LoadPortOriginSearchParameter parameter,
            IMacroCommandSender sender,
            IEquipmentFacade eqFacade)
        {
            return new OriginSearchCommand(
                RorzeConstants.CommandTypeAbb.Order,
                RorzeConstants.DeviceTypeAbb.LoadPort,
                deviceId,
                sender,
                eqFacade,
                ((int)parameter).ToString("X1"));
        }
        ...
        private OriginSearchCommand(
            ...
            : base(
                ...
                deviceType,
                deviceId,
                RorzeConstants.Commands.OriginSearch,
                sender,
                eqFacade,
                parameters)
        {
        ...
        protected override bool TreatEvent(RorzeMessage message)
        {
            ...
            switch (message.DeviceType)
            {
                case RorzeConstants.DeviceTypeAbb.LoadPort:
                    isDone = new RV201LoadPortStatus(message.Data).OperationStatus
                             == RorzeLoadPort.Driver.Enums.OperationStatus.Stop;
                    break;
