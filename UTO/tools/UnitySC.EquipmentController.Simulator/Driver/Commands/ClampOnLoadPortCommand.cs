...
namespace UnitySC.EquipmentController.Simulator.Driver.Commands
{
    public class ClampOnLoadPortCommand : LoadPortCommand
    {
        ...
        public static ClampOnLoadPortCommand NewOrder(
            ...
            ILogger logger)
        {
            var parameters = new List<string[]> { new[] { ((int)loadPort).ToString(), isReverseOperation ? "0" : "1" } };

            return new ClampOnLoadPortCommand(
        ...
        private ClampOnLoadPortCommand(
            char commandType,
            Constants.Port loadPort,
            ...
            IEquipmentFacade eqFacade,
            ILogger logger)
            : base(commandType, Constants.Commands.ClampOnLp, loadPort, parameters, sender, eqFacade, logger)
        {
        }
        ...
        {
            // If not an "order" command, we have no reason to treat the dock event
            ...
            // Success: eLPCP:<Port>_<switch>
            // Error:   eLPCP:<Port>_2_<Level>_<ErrCode>_<Msg>
