...
public:
	...
	template <typename T> inline void init_feed(T value, int slot) {
        if (std::isnan(value)) {
	...
	template <> inline void init_feed<uint16_t>(uint16_t value, int slot) {
		sorted[size] = std::make_pair(value, slot);
		++size;
	}

	template <> inline void init_feed<uint8_t>(uint8_t value, int slot) {
		sorted[size] = std::make_pair(value, slot);
		++size;
	}
...
template <typename T>
void median_filter_impl_2d(int x, int y, int hx, int hy, int b, int cores, const T* in, T* out) {
    if (2 * hx + 1 > b || 2 * hy + 1 > b) {
        throw std::invalid_argument("window too large for this block size");
    }

	if (cores == 0) { //we use all core available
	...
	else {// we only use a part of core available
...
template <typename T>
void median_filter_impl_1d(int x, int hx, int b, int cores, const T* in, T* out) {
    if (2 * hx + 1 > b) {
        throw std::invalid_argument("window too large for this block size");
    }
...
void median_filter_2d(int x, int y, int hx, int hy, int blockhint, int nCores, const T* in, T* out) {
    ...
    median_filter_impl_2d<T>(x, y, hx, hy, blocksize, nCores, in, out);
}
...
void median_filter_1d(int x, int hx, int blockhint, int nCores, const T* in, T* out) {
    ...
    median_filter_impl_1d<T>(x, hx, blocksize, nCores, in, out);
}

// OPTIMIZED EXECUTION
template void median_filter_2d<float>(int x, int y, int hx, int hy, int blockhint, int nCores, const float* in, float* out);
template void median_filter_2d<double>(int x, int y, int hx, int hy, int blockhint, int nCores, const double* in, double* out);
template void median_filter_1d<float>(int x, int hx, int blockhint, int nCores, const float* in, float* out);
template void median_filter_1d<double>(int x, int hx, int blockhint, int nCores, const double* in, double* out);

//Good but NON-OPTIMIZED EXECUTION -- need to make a new Window class optimized for uint16 (using 32 bits words instead of 64 bits) -- if squared window 8bits use ctmf algorithm
template void median_filter_2d<uint16_t>(int x, int y, int hx, int hy, int blockhint, int nCores, const uint16_t* in, uint16_t* out);
template void median_filter_2d<uint8_t>(int x, int y, int hx, int hy, int blockhint, int nCores, const uint8_t* in, uint8_t* out);
template void median_filter_1d<uint16_t>(int x, int hx, int blockhint, int nCores, const uint16_t* in, uint16_t* out);
template void median_filter_1d<uint8_t>(int x, int hx, int blockhint, int nCores, const uint8_t* in, uint8_t* out);