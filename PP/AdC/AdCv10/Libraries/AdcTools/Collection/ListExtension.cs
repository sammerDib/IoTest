...
namespace AdcTools
{
    public static class ListExtension
    {
        ...
        public static void SortByList<T>(this List<T> list, List<T> order)
        {
            ...
            List<T> orderedList = new List<T>(ordered);
            list.Clear();
        ...
        {
            foreach (KeyValuePair<K, V> kvp in dico2)
        ...
        {
            TValue val;
            bool exists = dico.TryGetValue(key, out val);
            if (!exists)
            {
                val = new TValue();
                dico.Add(key, val);
