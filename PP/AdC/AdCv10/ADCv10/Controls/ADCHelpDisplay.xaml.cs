...
{
    /// <summary>
    /// Interaction logic for ADCHelpDisplay.xaml
    /// </summary>
    public partial class ADCHelpDisplay : UserControl
    {
        ...
        public ADCHelpDisplay()
        {
            InitializeComponent();
            _helpFolder = ConfigurationManager.AppSettings["Editor.HelpFolder"];
            if (!Path.IsPathRooted(_helpFolder))
            {
                _helpFolder = Path.Combine(PathString.GetExecutingAssemblyPath().Directory, _helpFolder);
            }
        ...
        public static readonly DependencyProperty HelpNameProperty =
            DependencyProperty.Register("HelpName", typeof(string), typeof(ADCHelpDisplay), new FrameworkPropertyMetadata(null, OnHelpChanged));
        ...
        public static readonly DependencyProperty NavigateToPageInMainHelpProperty =
            DependencyProperty.Register("NavigateToPageInMainHelp", typeof(bool), typeof(ADCHelpDisplay), new PropertyMetadata(false));
        ...
        public static void OnHelpChanged(DependencyObject obj, DependencyPropertyChangedEventArgs e)
        {
            ...
            ADCHelpDisplay helpDisplay = GetHelpDisplay(obj);
        ...
        public static readonly DependencyProperty AutoResizeProperty =
            DependencyProperty.Register("AutoResize", typeof(bool), typeof(ADCHelpDisplay), new PropertyMetadata(false));
        ...
        public static readonly DependencyProperty ShowMoreProperty =
            DependencyProperty.Register("ShowMore", typeof(bool), typeof(ADCHelpDisplay), new PropertyMetadata(true));
        ...
        /// <param name="sender"></param>
        /// <returns></returns>
        private static ADCHelpDisplay GetHelpDisplay(DependencyObject sender)
        {
            ADCHelpDisplay helpDisplay = null;
            if (sender is ADCHelpDisplay)
                helpDisplay = (ADCHelpDisplay)sender;
            if (helpDisplay == null)
        ...
        private void Resize()
        {
            if (AutoResize)
            {
                ...
                WebBrowser browser = webHelpBrowser as WebBrowser;
                ...
                mshtml.HTMLHeadElement head = doc.getElementsByTagName("head").OfType<mshtml.HTMLHeadElement>().First();
        ...
        public static void OpenMainHelp(string helpName = null)
        {
            ...
            string mainHelpPath = Path.Combine(ConfigurationManager.AppSettings["Editor.HelpFolder"], MainHelpFolder);
            ...
            if (!string.IsNullOrEmpty(helpName) && Directory.Exists(mainHelpPath))
            {
                ...
                if (helpName.ToLower().StartsWith("func_")) // Page de catégorie
                    pageName = helpName + ".htm";
                else // page de module
                    pageName = "Md_" + helpName.Remove(helpName.LastIndexOf('_')) + ".htm";
                PathString filePath = Directory.EnumerateFiles(mainHelpPath, pageName, SearchOption.AllDirectories).FirstOrDefault();
                if (!string.IsNullOrEmpty(filePath))
                {
                    urlPath = String.Format("{0}#t={1}", urlPath, filePath.GetSubPath(mainHelpPath));
                }
            }
