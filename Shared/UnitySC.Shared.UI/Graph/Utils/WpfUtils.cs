...
namespace UnitySC.Shared.UI.Graph.Utils
{
    ...
    public static class WpfUtils
    {
        ...
        public static ParentT FindVisualParentWithType<ParentT>(FrameworkElement childElement)
            where ParentT : class
        {
            var parentElement = VisualTreeHelper.GetParent(childElement) as FrameworkElement;
            if (parentElement != null)
            {
        ...
        public static T HitTestHighestForDataContext<T>(FrameworkElement rootElement, Point point)
            where T : class
        {
            return HitTestHighestForDataContext<T>(rootElement, point, out var hitFrameworkElement);
        }
