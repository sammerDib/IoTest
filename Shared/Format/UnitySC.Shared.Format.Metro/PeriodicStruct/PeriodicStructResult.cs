...
{
    public class PeriodicStructResult : MeasureResultBase
    {
        #region Properties
        ...
        [DataMember]
        public PeriodicStructResultSettings Settings { get; set; } = new PeriodicStructResultSettings();
        ...
        public override void FillNonSerializedProperties(bool generateState, bool generateStatistics)
        {
            ...
            {
                if (!(measurePointResult is PeriodicStructPointResult point)) return;

                foreach (var data in point.PeriodicStructDatas)
                {
        ...
        public override byte[] ToCsv()
        {
            ...
            if (!isDieResult)
            {
                var measurePointResults = Points.OfType<PeriodicStructPointResult>().ToList();
                bool hasPitch = measurePointResults.Any(result => result.PitchStat != null);
                ...
                if (hasPitch)
                {
                    AppendHeader("Pitch", hasRepeta, ref sbCSV, " (µm)");
                }
                ...
                for (int i = 0; i < measurePointResults.Count; i++)
                {
                    ...
                    if (hasPitch)
                    {
                        AppendStatsMicrometers(p.PitchStat, hasRepeta, ref sbCSV);
                    }
                    sbCSV.RemoveEndDelim();
                    lines.Add(sbCSV.ToString());
                }
            ...
            else
            {
                ...
                bool hasPitch = Dies.SelectMany(r => r.Points).Any(re => re.GetType() == typeof(PeriodicStructPointResult));
                ...
                {
                    foreach (var p in die.Points.OfType<PeriodicStructPointResult>())
                    {
                    	sbCSV.Clear();
