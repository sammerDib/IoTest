...
{
    /// <summary>
    /// Summary description for TrenchTest
    /// </summary>
    [TestClass]
    public class TrenchTest
    {
        [TestMethod]
        public void TestTrenchFileTypeConsistency()
        {
            string FileNameIn = @"Wafer1.anatch";
            ...
            AssertEx.Throws<Exception>(() => new MetroResult(UnitySC.Shared.Data.Enum.ResultType.ANALYSE_Trench, -1, BadFileNameIn)); // Result Extension id not matched
        }

        [TestMethod]
        public void ReadWriteTrenchBlankWaferResult()
        {
            var TrenchResult = new TrenchResult();
            TrenchResult.Name = "TrenchTest1";
            TrenchResult.Settings.DepthTarget = 45.Micrometers();
            TrenchResult.Settings.DepthTolerance = new LengthTolerance(2, Tools.Tolerances.LengthToleranceUnit.Micrometer);
            TrenchResult.Settings.WidthTarget = 120.Micrometers();
            TrenchResult.Settings.WidthTolerance = new LengthTolerance(8, Tools.Tolerances.LengthToleranceUnit.Micrometer);


            TrenchResult.Information = "Test blank wafer result";
            var rnd = new Random();
            TrenchResult.State = (GlobalState)rnd.Next(0, 5);
            TrenchResult.Wafer = MetroHelper.CreateWafer();
            TrenchResult.Points = new List<MeasurePointResult>();

            TrenchResult.Points.Add(CreateTestTrenchPointResult());
            TrenchResult.Points.Add(CreateTestTrenchPointResult());
            TrenchResult.Points.Add(CreateTestTrenchPointResult());
            TrenchResult.Points.Add(CreateTestTrenchPointResult());
            TrenchResult.Points.Add(CreateTestTrenchPointResult());
            ...
            var metroResWrite = new MetroResult(Data.Enum.ResultType.ANALYSE_Trench);
            metroResWrite.MeasureResult = TrenchResult;

            // Test WCF Serialize
            XML.DatacontractSerialize(TrenchResult, "TrenchBlankWafer.testwcf");
            var resWCF = XML.DatacontractDeserialize<TrenchResult>("TrenchBlankWafer.testwcf");

            string error;
            Assert.IsFalse(metroResWrite.WriteInFile(@"TrenchBlankWafer.xml", out error), "Bad file name extension for Trench results");

            bool bRes = metroResWrite.WriteInFile(@"TrenchBlankWafer.anatch", out error);
            Assert.IsTrue(string.IsNullOrEmpty(error));

            var metroRes = new MetroResult(Data.Enum.ResultType.ANALYSE_Trench);
            metroRes.ReadFromFile(@"TrenchBlankWafer.anatch", out error);
            Assert.IsTrue(string.IsNullOrEmpty(error));

            var resXML = metroRes.MeasureResult as TrenchResult;
            AssertResult(TrenchResult, resXML, "XML Serialize");

            AssertResult(TrenchResult, resWCF, "WCF Serialize");
        }

        private MeasurePointResult CreateTestTrenchPointResult(bool isInDie = false)
        {
            var point = new TrenchPointResult();
            ...
            var pointData1 = new TrenchPointData();
            pointData1.Width = (MetroHelper.StaticRandom.Instance.Next(0, 2000) * 0.01).Micrometers();
            pointData1.Depth = (MetroHelper.StaticRandom.Instance.Next(0, 500) * 0.01).Micrometers();
            pointData1.QualityScore = MetroHelper.StaticRandom.Instance.Next(0, 100) * 0.01;

            // RAW PROFILE
            pointData1.RawProfileScan = createRawProfileScan(pointData1.Width, pointData1.Depth);

            point.Datas.Add(pointData1);
            var pointData2 = new TrenchPointData();

            pointData2.Width = (MetroHelper.StaticRandom.Instance.Next(0, 2000) * 0.01).Micrometers();
            pointData2.Depth = (MetroHelper.StaticRandom.Instance.Next(0, 500) * 0.01).Micrometers();
            pointData2.QualityScore = MetroHelper.StaticRandom.Instance.Next(0, 100) * 0.01;
            pointData2.IndexRepeta = 1;

            // RAW PROFILE
            pointData2.RawProfileScan = createRawProfileScan(pointData2.Width, pointData2.Depth);

            point.Datas.Add(pointData2);
            return point;
        }

        private void AssertResult(TrenchResult expected, TrenchResult actual, string testName)
        {
            ...
            if (expected.Points != null)
            {
                ...
                for (int i = 0; i < expected.Points.Count; i++)
                {
                    ...
                    Assert.AreEqual(((expected.Points[i]) as TrenchPointResult).ScanAngle.ToString(), ((actual.Points[i]) as TrenchPointResult).ScanAngle.ToString(), $"{testName} Points[{i}].ScanAngle");
                    ...
                    {
                        var expectedData = (expected.Points[i].Datas[j] as TrenchPointData);
                        var actualData = (actual.Points[i].Datas[j] as TrenchPointData);
            ...
            if (expected.Dies != null)
            {
                ...
                for (int i = 0; i < expected.Dies.Count; i++)
                {
                    ...
                    for (int j = 0; j < expected.Dies[i].Points.Count; j++)
                    {
                        ...
                        Assert.AreEqual(((exp) as TrenchPointResult).ScanAngle.ToString(), ((act) as TrenchPointResult).ScanAngle.ToString(), $"{testName} Points[{i}].ScanAngle");


                        Assert.AreEqual(exp.Datas.Count, act.Datas.Count, $"{testName} Dies[{i}].Points[{j}].Datas.Count");
                        for (int k = 0; k < exp.Datas.Count; k++)
                        {
                            var expectedData = (exp.Datas[k] as TrenchPointData);
                            var actualData = (act.Datas[k] as TrenchPointData);
        ...
        private RawProfile createRawProfileScan(Length X, Length Z)
        {
            ...
            double Zlo_nm = MetroHelper.StaticRandom.Instance.Next(-15, 0);
