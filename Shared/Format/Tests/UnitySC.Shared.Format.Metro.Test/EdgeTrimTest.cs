...
{
    [TestClass]
    public class EdgeTrimTest
    {
        [TestMethod]
        public void TesEdgeTrimFileTypeConsistency()
        {
            string FileNameIn = @"Wafer1.anaedt";
            ...
            AssertEx.Throws<Exception>(() => new MetroResult(UnitySC.Shared.Data.Enum.ResultType.ANALYSE_EdgeTrim, -1, BadFileNameIn)); // Result Extension id not matched
        }

        [TestMethod]
        public void ReadWriteStepBlankWaferResult()
        {
            var EdgeTrimResult = new EdgeTrimResult();
            EdgeTrimResult.Name = "EdgeTrimTest1";
            EdgeTrimResult.Settings.HeightTarget = 20.Micrometers();
            EdgeTrimResult.Settings.HeightTolerance = new LengthTolerance(2, Tools.Tolerances.LengthToleranceUnit.Micrometer);
            EdgeTrimResult.Settings.WidthTarget = 5.Micrometers();
            EdgeTrimResult.Settings.WidthTolerance = new LengthTolerance(2, Tools.Tolerances.LengthToleranceUnit.Micrometer);
            EdgeTrimResult.Settings.StepUp = (MetroHelper.StaticRandom.Instance.Next(0, 10) % 2) == 0;
            EdgeTrimResult.Information = "Test blank wafer result";
            var rnd = new Random();
            EdgeTrimResult.State = (GlobalState)rnd.Next(0, 5);
            EdgeTrimResult.Wafer = MetroHelper.CreateWafer();
            EdgeTrimResult.Points = new List<MeasurePointResult>
            {
                CreateTestEdgeTrimPointResult(EdgeTrimResult.Settings.StepUp, false),
                CreateTestEdgeTrimPointResult(EdgeTrimResult.Settings.StepUp, false),
                CreateTestEdgeTrimPointResult(EdgeTrimResult.Settings.StepUp, false),
                CreateTestEdgeTrimPointResult(EdgeTrimResult.Settings.StepUp, false),
                CreateTestEdgeTrimPointResult(EdgeTrimResult.Settings.StepUp, false),
            ...
            var metroResWrite = new MetroResult(Data.Enum.ResultType.ANALYSE_EdgeTrim);
            metroResWrite.MeasureResult = EdgeTrimResult;

            // Test WCF Serialize
            XML.DatacontractSerialize(EdgeTrimResult, "EdgeTrimBlankWafer.testwcf");
            var resWCF = XML.DatacontractDeserialize<EdgeTrimResult>("EdgeTrimBlankWafer.testwcf");

            string error;
            Assert.IsFalse(metroResWrite.WriteInFile(@"EdgeTrimBlankWafer.xml", out error), "Bad file name extension for Step results");

            bool bRes = metroResWrite.WriteInFile(@"EdgeTrimBlankWafer.anaedt", out error);
            Assert.IsTrue(string.IsNullOrEmpty(error));

            var metroRes = new MetroResult(Data.Enum.ResultType.ANALYSE_EdgeTrim);
            metroRes.ReadFromFile(@"EdgeTrimBlankWafer.anaedt", out error);
            Assert.IsTrue(string.IsNullOrEmpty(error));

            var resXML = metroRes.MeasureResult as EdgeTrimResult;
            AssertEdgeTrimResult(EdgeTrimResult, resXML, "XML Serialize");

            AssertEdgeTrimResult(EdgeTrimResult, resWCF, "WCF Serialize");
        }

        private MeasurePointResult CreateTestEdgeTrimPointResult(bool stepup, bool isInDie)
        {
            var point = new EdgeTrimPointResult();
            ...
            var pointData1 = new EdgeTrimPointData();
            pointData1.Height = (MetroHelper.StaticRandom.Instance.Next(0, 2000) * 0.01).Micrometers();
            ...
            // RAW PROFILE
            pointData1.RawProfileScan = createRawProfileScan(10.Micrometers(), pointData1.Height);

            point.Datas.Add(pointData1);
            var pointData2 = new EdgeTrimPointData();
            pointData2.Height = (MetroHelper.StaticRandom.Instance.Next(0, 2000) * 0.01).Micrometers();
            ...
            // RAW PROFILE
            pointData2.RawProfileScan = createRawProfileScan(15.Micrometers(), pointData2.Height, stepup);

            point.Datas.Add(pointData2);
            return point;
        }

        private void AssertEdgeTrimResult(EdgeTrimResult expected, EdgeTrimResult actual, string testName)
        {
            ...
            if (expected.Points != null)
            {
                ...
                for (int i = 0; i < expected.Points.Count; i++)
                {
                    ...
                    Assert.AreEqual(((expected.Points[i]) as EdgeTrimPointResult).ScanAngle.ToString(), ((actual.Points[i]) as EdgeTrimPointResult).ScanAngle.ToString(), $"{testName} Points[{i}].ScanAngle");
                    ...
                    {
                        var expectedData = (expected.Points[i].Datas[j] as EdgeTrimPointData);
                        var actualData = (actual.Points[i].Datas[j] as EdgeTrimPointData);
            ...
            if (expected.Dies != null)
            {
                ...
                for (int i = 0; i < expected.Dies.Count; i++)
                {
                    ...
                    for (int j = 0; j < expected.Dies[i].Points.Count; j++)
                    {
                        ...
                        Assert.AreEqual(((exp) as EdgeTrimPointResult).ScanAngle.ToString(), ((act) as EdgeTrimPointResult).ScanAngle.ToString(), $"{testName} Points[{i}].ScanAngle");


                        Assert.AreEqual(exp.Datas.Count, act.Datas.Count, $"{testName} Dies[{i}].Points[{j}].Datas.Count");
                        for (int k = 0; k < exp.Datas.Count; k++)
                        {
                            var expectedData = (exp.Datas[k] as EdgeTrimPointData);
                            var actualData = (act.Datas[k] as EdgeTrimPointData);
