...
namespace UnitySC.Shared.ResultUI.Metro.ViewModel.Stats.Thickness
{
    public class ThicknessStatsVM : MetroStatsVM
    {
        #region Properties
        ...
        protected override void RedrawChart()
        {
            ...
            var distinctOutputName = layers.Select(pair => pair.Key).Distinct().Take(3);
            var outputToDisplay = distinctOutputName.Select(output => layers.First(pair => string.Equals(pair.Key, output))).ToList();
            ...
            GeneratedColumns.Add(new GeneratedColumn
            {
                ...
                ValueBinding = new MultiBinding
                {
                    ...
                    Bindings =
                    {
                        ...
                        new Binding
                        {
                            Source = new Func<WaferResultData, string>(result => LengthToStringConverter.ConvertToString(getValueFunc(result), 5, false, "-", LengthUnit.Nanometer)),
                        },
