...
{
    public class PillarDataRepetaVM : MetroDataRepetaVM<PillarPointData>
    {
        #region Properties
        ...
        public PillarDataRepetaVM(PointSelectorBase pointSelector) : base(pointSelector)
        {
        }
        ...
        {
            var point = measurePointResult as PillarPointResult;
            var statsContainer = ExtractStats(point, SelectedOutput);

            var unit = LengthUnit.Undefined;
            if (SelectedOutput == PillarResultVM.HeightOutputName || SelectedOutput == PillarResultVM.WidthOutputName) unit = LengthUnit.Micrometer;
        ...
        private static IStatsContainer ExtractStats(PillarPointResult point, string selectedOutput)
        {
        ...
        public void UpdateOutputSource(List<string> newSource)
        {
            ...
            {
                Func<PillarPointData, double?> getValue;
                Func<PillarPointData, MeasureState> getState;
                string columnName;
                switch (output)
                {
                    // Defined in Xaml
        ...
        private void GenerateColumn(string columnName, Func<PillarPointData, double?> getValueFunc, Func<PillarPointData, MeasureState> getStateFunc)
        {
            GeneratedColumns.Add(new GeneratedStateColumn
            {
                ...
                ValueBinding = new MultiBinding
                {
                    ...
                    Bindings =
                    {
                        ...
                        new Binding
                        {
                            Source = new Func<PillarPointData, int, string>((result, digits) => LengthToStringConverter.ConvertToString(getValueFunc(result), digits, false, "-", LengthUnit.Micrometer)),
                        },
