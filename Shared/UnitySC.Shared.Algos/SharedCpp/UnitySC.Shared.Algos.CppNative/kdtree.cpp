...
void KdTree::k_nearest_neighbors(const CoordPoint& point, size_t k,
                                 KdNodeVector* result,
                                 KdNodePredicate* searchpredicate /*=NULL*/) {
  size_t i;
  KdNode temp;
  ...
  if (k > allnodes.size()) {
    ...
    neighbor_search(point, root, k, neighborheap, searchpredicate);
  }
...
bool KdTree::neighbor_search(const CoordPoint& point, kdtree_node* node,
  ...
  if (point[node->cutdim] < node->point[node->cutdim]) {
    if (node->loson)
      if (neighbor_search(point, node->loson, k, neighborheap, searchpredicate)) return true;
  } else {
    if (node->hison)
      if (neighbor_search(point, node->hison, k, neighborheap, searchpredicate)) return true;
  }
