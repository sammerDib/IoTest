...
namespace filter {
    ...
    /**
     * Shen smoothing by Garcia Lorca operators (low-pass filter).
     * See DericheBlur for more details.
     *
     * @param img     - input two-dimensional signal
     * @param gamma   - γ = exp(-α)
     *
     * @return Filtered image
     */
    cv::Mat ShenBlur(const cv::Mat& img, float gamma);

    /**
     * Canny filter using Scharr derivatives
     *
     * @param img               - input two-dimensional signal
     * @param cannyThreshold    - canny threshold (default = 300)
     *
     * @return Filtered image
     */
    cv::Mat CannyScharr(const cv::Mat& img, int cannyThreshold);

    /**
     * Canny filter using Sobel derivatives
     *
     * @param img               - input two-dimensional signal
     * @param cannyThreshold    - canny threshold (default = 100)
     *
     * @return Filtered image
     */
    cv::Mat CannySobel(const cv::Mat& img, int cannyThreshold);
    ...
    */
    ...
    /**
    * Create a masked image using the Otsu Binarization of the original image, only copying pixels that are not white in the binarization
    *
    * @param src - Source image
    * @return the masked image
    */
    void NoiseRemovedBelowThreshold(cv::Mat& src);
} // namespace filter