...
Status HyperAccurateCircleFitter::Fit(std::vector<cv::Point2d> const &points, ICircleFitter::Result &fit) const {
...
Status HyperAccurateCircleFitter::BuildStatus(const ICircleFitter::Result &fit) const {
  ...
  } else if (fit.rmse > rmseThreshold) {
    status.code = StatusCode::BWA_FIT_FAILED;
    std::ostringstream stringStream;
