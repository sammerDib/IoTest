...
public:
  ...
  struct TakenImage : public Image {
    ...
    EdgeImage::Pointer ToEdgeImage() {
        auto instPixelSize = EdgeImage::PixelSize(pixelSize);
        auto instCentroid = EdgeImage::ImageCentroid(centroid);
        EdgeImage::Pointer image = EdgeImage::New(std::string(label), content.clone(), instPixelSize, instCentroid);
        return image;
    }

    NotchImage::Pointer ToNotchImage() {
        auto instPixelSize = NotchImage::PixelSize(pixelSize);
        auto instCentroid = NotchImage::ImageCentroid(centroid);
        NotchImage::Pointer image = NotchImage::New(std::string(label), content.clone(), instPixelSize, instCentroid);
        return image;
    }
  };
  struct Result {
