...
namespace phase_unwrapping {
    ...
    Mat QualityGuidedUnwrap(Mat& wrappedPhaseMap, QualityMode mode) {
        if (wrappedPhaseMap.type() != CV_32FC1) {
            ErrorLogging::LogError("[Quality guided unwrap] Input phase map should be stored into a single channel 32-bits float");
            return Mat();
        }
        ...
        if (!validRangeValues) {
            ErrorLogging::LogError("[Quality guided unwrap] Input phase map should be wrapped between [-pi, pi]");
            return Mat();
        }
    ...
    namespace {
        ...
        Mat QualityMap(Mat& wrappedPhaseMap, Mat& bitFlags, QualityMode mode) {
            ...
            if (mode == QualityMode::PseudoCorrelation) {
                ...
                for (int pixelId = 0; pixelId < size; pixelId++) {
                    if (!bitFlags.empty() && (bitFlags.at<uchar>(pixelId) & BORDER))
                        qualityMap.at<float>(pixelId) = 0.0f;
                }
            }
            ...
            // PHASE VARIANCE & PHASE GRADIENT
                ...
                auto scale = (float)((min != max) ? 1.0 / (max - min) : 0.0);
                float fmax = (float) max;
                for (int pixelId = 0; pixelId < size; pixelId++) {
                    qualityMap.at<float>(pixelId) = (fmax - qualityMap.at<float>(pixelId)) * scale;
                    if (!bitFlags.empty() && (bitFlags.at<uchar>(pixelId) & BORDER))
                        qualityMap.at<float>(pixelId) = 0.0f;
                }
            }
        ...
        void PseudoCorrelationQualityMap(Mat& wrappedPhaseMap, Mat& qualityMap, int kernelSize) {
            ...
            for (int pixelId = 0; pixelId < size; pixelId++) {
                temp.at<float>(pixelId) = static_cast<float>(cos(CV_PI2 * wrappedPhaseMap.at<float>(pixelId))); // 2pi ou pi2 ?
            }
            ...
            // Extracting sin's
            for (int pixelId = 0; pixelId < size; pixelId++) {
                temp.at<float>(pixelId) = static_cast<float>(sin(CV_PI2 * wrappedPhaseMap.at<float>(pixelId))); // 2pi ou pi2 ?
            }
            ...
            // Square root
            for (int pixelId = 0; pixelId < size; pixelId++)
                qualityMap.at<float>(pixelId) = 1.0f - sqrt(qualityMap.at<float>(pixelId));
        }
