...
namespace phase_unwrapping {
    ...
    Mat ReliabilityHistUnwrap::UnwrapPhaseMap(Mat& wrappedPhaseMap, Mat& mask) {
        ...
        if (wrappedPhaseMap.type() != CV_32FC1) {
            ErrorLogging::LogError("[Reliability histogram unwrap] Input phase map should be stored into a single channel 32-bits float.");
            return Mat();
        }

        if (mask.type() != CV_8UC1) {
            ErrorLogging::LogError("[Reliability histogram unwrap] Input mask should be stored into a single channel 8-bits unsigned char.");
            return Mat();
        }
        ...
        if (!validRangeValues) {
            ErrorLogging::LogError("[Reliability histogram unwrap] Input phase map should be wrapped between [-pi, pi]");
            return Mat();
        }
    ...
    Mat ReliabilityHistUnwrap::InverseReliabilityMap() {
        ...
        for (int i = 0; i < rows; ++i) {
            for (int j = 0; j < cols; ++j) {
                int idx = i * cols + j;
                //int rel = _pixelsMap.Pixels.at(idx).InverseReliability; // not used
